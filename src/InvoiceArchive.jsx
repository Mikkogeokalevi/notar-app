...
const InvoiceArchive = ({ onBack, showNotification, requestConfirm }) => {
    ...
    // --- APUFUNKTIOT ---
    // Suomalainen viitenumero: 7-3-1, väh. 4 numeroa, EI ala nollalla (pankit typistävät alkuperän)
    const generateReferenceNumber = (invoiceNum) => {
        if (!invoiceNum) return "";
        const num = parseInt(String(invoiceNum).replace(/\D/g, ''), 10) || 0;
        const base = String(1000 + num); // 1001, 1002, ... → ei koskaan 0-alkuinen
        const weights = [7, 3, 1];
        let sum = 0;
                            ...
                            <td>
                                <div class="footer-content">
                                    <div style="display:flex; justify-content:space-between; font-size:12px;">
                                        ...
                                        <div style="text-align:right"><b>Viite:</b> ${refNum}<br><b>Eräpäivä:</b> ${dueDate}<br><b>Summa:</b> ${inv.total_sum.toFixed(2)} €</div>
                                    </div>
                        ...
                        <tr>
                            <td>
                                ...
                                <div class="totals-box">
                                    <div class="total-row"><span>VEROTON:</span> <span>${totalNet.toFixed(2)} €</span></div>
                                    <div class="total-row"><span>ALV ${alvRate}%:</span> <span>${totalVat.toFixed(2)} €</span></div>
                                    <div class="total-final"><span>YHTEENSÄ:</span> <span>${inv.total_sum.toFixed(2)} €</span></div>
                                </div>
                            </td>
                        </tr>
                    </tbody>
